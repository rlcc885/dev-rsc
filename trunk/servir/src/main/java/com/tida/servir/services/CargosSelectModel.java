...
/** Cargos selection model for a list of Objects.
 * use:
...
public class CargosSelectModel<T> extends AbstractSelectModel implements ValueEncoder<T> {

    private PropertyAdapter labelFieldAdapter1;
    private PropertyAdapter labelFieldAdapter2;
    private PropertyAdapter idFieldAdapter;
    private Collection<T>         list;
    List<OptionGroupModel> optionGroups;

    public CargosSelectModel(Collection<T> list, Class<T> clazz, String labelField1, String labelField2, String idField, PropertyAccess access) {
        this.list = list;
        if (idField != null)
            this.idFieldAdapter = access.getAdapter(clazz).getPropertyAdapter(idField);
        if (labelField1 != null)
            this.labelFieldAdapter1 = access.getAdapter(clazz).getPropertyAdapter(labelField1);
        if (labelField2 != null)
            this.labelFieldAdapter2 = access.getAdapter(clazz).getPropertyAdapter(labelField2);
    }
    ...
    public void addOptionGroup(String label, boolean disabled, List<T> options) {
        ...
        if ((labelFieldAdapter1 == null) || (labelFieldAdapter2 == null)) {
            for (T obj : options) {
                optionModels.add(new OptionModelImpl(nvl(obj), obj));
            }
        } else {
            for (T obj : options) {
                optionModels.add(new OptionModelImpl(nvl(labelFieldAdapter1.get(obj)) + " - " +nvl(labelFieldAdapter2.get(obj)), obj));
            }
        }
    ...
    public List<OptionModel> getOptions() {
        ...
        if ((labelFieldAdapter1 == null) || (labelFieldAdapter2 == null)) {
            ...
            for (T obj : list) {
                optionModelList.add(new OptionModelImpl(nvl(labelFieldAdapter1.get(obj)) + " - " + nvl(labelFieldAdapter2.get(obj)), obj));
            }
        }
