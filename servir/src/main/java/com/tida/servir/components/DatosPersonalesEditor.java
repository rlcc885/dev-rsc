...
public class DatosPersonalesEditor {
    ...
    void onValidateFromformulariodatospersonales(){
    	if(actual.fechaNacimiento.after(new Date())) {
	        ...
	        if(getAge(actual.fechaNacimiento, new Date()) < ca.getEdad_minima()) {
    ...
    Object onSuccessFromformulariodatospersonales()
    {
        ...
        List<Trabajador> lTrabajador = session.createCriteria(Trabajador.class)
            .add(Restrictions.eq("nroDocumento", actual.getNroDocumento()))
            .add(Restrictions.ne("id", actual.getId()))
            .list();
    ...
    MultiZoneUpdate tresZonas() {
        MultiZoneUpdate mu;
        mu = new MultiZoneUpdate("ubigeoNacZone", ubigeoNacZone.getBody())
                .add("ubigeoDomZone", ubigeoDomZone.getBody());
        return mu;
    }
    ...
    private static int getAge(Date bDay, Date now) { int res = now.getYear() - bDay.getYear(); if ((bDay.getMonth() > now.getMonth()) || (bDay.getMonth() == now.getMonth() && bDay.getDate() > now.getDate())) { res--; } return res; }
